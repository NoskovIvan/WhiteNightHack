...
from parking_schema import PARKING_JOURNEY_MODEL
...
class ParkingAgent(OEFAgent):
    """Class that implements the behaviour of the parking agent."""

    parking_description = Description(
        {
            "price_per_1hour": True,
        },
        PARKING_JOURNEY_MODEL
    )

    def __init__(self, *args, **kwargs):
      super(ParkingAgent, self).__init__(*args, **kwargs)
...
#      self.prepare_contract()
    ...
    def on_cfp(self, msg_id: int, dialogue_id: int, origin: str, target: int, query: CFP_TYPES):
        ...
        proposal = Description({"price_per_1hour": price})
...
if __name__ == "__main__":
    agent = ParkingAgent("parking", oef_addr="127.0.0.1", oef_port=10000)
    agent.connect()
    agent.register_service(77, agent.parking_description)
